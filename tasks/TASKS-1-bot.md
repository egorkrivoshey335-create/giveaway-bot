...
**Не реализовано:**
- ❌ **Webhook mode** (только long polling) - см. Задачу 1.1
- ❌ **Нет ENV BOT_MODE** переключателя - см. Задачу 1.1
...
- ⚠️ **НЕ РЕАЛИЗОВАНО**: остальные 11+ jobs из полного списка
...
- ❌ `subscription:expire` - истечение платных подписок
- ❌ `subscription:auto-renew` - автопродление
- ❌ `subscription:expire-warning` - предупреждение за 3 дня
- ❌ `subscription:deactivate` - деактивация
- ❌ `creator:milestone` - milestone уведомления (10/50/100...)
- ❌ `sandbox:cleanup` - очистка sandbox розыгрышей
- ❌ `liveness:cleanup` - удаление старых liveness фото
- ❌ `badges:check` - пересчет бейджей
- ❌ `prize-form:cleanup` - очистка форм получения приза
...
- ❌ НЕТ настроек в боте для переключения режима уведомлений
- ❌ Поле `User.creatorNotificationMode` существует в Prisma, но не используется
- ⚠️ Требуется UI в боте для управления настройками
