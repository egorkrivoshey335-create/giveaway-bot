...
### [x] Задача 1.3 — "Создать розыгрыш" (в боте)
**Что подразумевает:**
...
**Решение:** Создание розыгрыша реализовано ТОЛЬКО через Mini App — это правильный архитектурный выбор. Пошаговый conversation flow в боте (10+ шагов) сложнее и хуже UX чем визуальный wizard в приложении. Кнопка "В боте" намеренно показывает "🔜 Скоро".
...
### [x] Задача 1.15 — Глобальная навигация бота
**Что подразумевает:**
...
**Реализовано (обновлено 2026-02-17):**
- ✅ `addNavigationButtons()` helper создан в `apps/bot/src/lib/navigation.ts`
- ✅ Используется в `handlers/giveaways.ts`
- ✅ `channels.ts` и `posts.ts` имеют встроенные nav кнопки (`back_to_menu`, `go_to_menu`)
- ✅ Menu stack в памяти — `bot.ts:74-91` (`userMenuStack`)
- ✅ Callbacks `back_to_menu`, `go_to_menu` — `bot.ts:353-366`
