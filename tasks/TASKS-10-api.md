...
### [x] Задача 10.3 — API маршруты: Каналы
**Статус:** Частично реализовано
...
- Каналы добавляются ТОЛЬКО через бота (`POST /internal/channels/upsert`) — намеренный дизайн. Бот парсит форвард/ссылку, проверяет права и добавляет. Это правильный UX поток.
...
### [x] Задача 10.4 — API маршруты: Посты (шаблоны)
**Статус:** Частично реализовано
...
**✅ Архитектурное решение:**
- Посты создаются ТОЛЬКО через бота (`POST /internal/post-templates/create`) — намеренный дизайн. Телеграм позволяет отправлять медиа только через бота. Из Mini App нет доступа к медиафайлам.
